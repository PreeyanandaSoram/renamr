...
export async function uppercase(files, options) {
  ...
  for (const file of matches) {
    ...
    const newBase = base.toUpperCase();
    const newPath = path.join(dir, newBase + ext.toUpperCase());
  ...
  console.log();
  console.log(chalk.gray(`Converted ${results.length} file(s) to uppercase`));
  if (options.dryRun) {
